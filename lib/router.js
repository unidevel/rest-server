...
function throwCode(code, body){
  var err = new error.HttpError(code, body);
  throw err;
}
...
class RestRouter {
  ...
  leave(){
    if ( this.target ){
      ...
      this.target.throwCode = throwCode;
    }
  ...
  middleware(){
    ...
    return function*(next){
      ...
      for ( var name in mapping ) {
        ...
        socket.on(name, function(target, name, args){
